...
class ExhaustiveSimulationMasses(Node):
    def __init__(self, ramp_angle=2, new_simulation=True, walker_counter=0, csv_filename='mass_simulation_result.csv'):
        ...
        self.SPAWN_Z_OFFSET = 5 * sin(self.RAMP_ANGLE_IN_RAD) + 0.5 # NOTE: 5m is the length of the ramp
        ...
        self.velocity_threshold = 0.01 # m/s  # TODO: experiment with this
        ...
        self.PATH_TO_URDF = os.path.join(get_package_share_directory('walker_design_4'), 'urdf')
        self.URDF_TREE = ET.parse(f'{self.PATH_TO_URDF}/walker_design_4.urdf')
        ...
        current_config = self.ALL_M_COMBINATION[self.walker_counter]
        current_urdf_tree = xml_functions.modify_mass_urdf(
            tree=self.URDF_TREE,
            mass_dict={'left_foot_link': current_config[1],
                       'right_foot_link': current_config[1],
                       'left_hip_link': current_config[0]}
        )
        ...
        run(['ros2', 'run', 'gazebo_ros', 'spawn_entity.py',
            ...
            '-y', '0.1', # move the walker a bit forward so it does not fall off the edge of the ramp 
            '-R', str(self.SPAWN_R)],
        ...
        if new_simulation:
            with open(self.CSV_PATH, 'w') as file:
                ...
                writer.writerow(['m_leg', 'm_hip', 'distance'])
    ...
    def link_states_callback(self, msg):
        ...
        if should_delete_entity:
            ...
            for link_name, pose in zip(msg.name, msg.pose):
                if link_name == 'walker::dummy_link':
                    ...
                    with open(self.CSV_PATH, 'a') as file:
                        writer = csv.writer(file)
                        masses = self.ALL_M_COMBINATION[self.walker_counter]
                        data_to_write = [masses[0], masses[1], d]
...
def main(args=None):
    ...
    exhaustive_simulation_joint_angles = ExhaustiveSimulationMasses(
        ...
        new_simulation=True,
        walker_counter=0,
        csv_filename='link_mass_simulation_result.csv'
    ) 
    try:
